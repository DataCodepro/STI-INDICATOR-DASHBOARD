...
def run():
    ...
    with col6:
        ...
        fig = px.scatter(df02,x='OUTCOME',y='RESPONSE',size='RESPONSE', color='OUTCOME',hover_name='INFO SOURCE',size_max=100,animation_frame='INFO SOURCE', title="IMPORTANCE OF INFORMATION SOURCES TO INNOVATION")
        fig.update_traces(hovertemplate =None)
        fig.update_layout(width =800,height=800,hovermode ='x unified',hoverlabel = dict(bgcolor = 'black',font_size =16,font_family = 'Rockwel'),showlegend =False)
        fig.update_layout(legend= dict(title_font_family = 'Rockwell',font_size =16))
        st.plotly_chart(fig)
